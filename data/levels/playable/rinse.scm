;; generated by xml2sexpr.rb from file 'branches/pingus_0_6/data/levels/playable/rinse.pingus'
(pingus-level 
  (version 2)
  (head 
    (levelname "Lather, Rinse, Repeat.")
    (description )
    (author )
    (number-of-pingus 40)
    (number-to-save 20)
    (time 9000)
    (comment "Pingu walk on air right above the plant. Also, Pingu get trapped in middle "
             "of plant and stuck on the left side of the map (at the top) too easily.")
    (actions 
      (basher 20)
      (blocker 10)
      (bomber 10)
      (bridger 20))
    (music "none")
    (levelsize 1024 600))
  (objects 
    (surface-background 
      (surface 
        (image "textures/enchantedjungle")
        (modifier "ROT0"))
      (color 0 0 0 1)
      (scroll-x 0.0)
      (scroll-y 0.0)
      (para-x 0.5)
      (para-y 0.5)
      (stretch-x 0)
      (stretch-y 0))
    (liquid 
      (surface 
        (image "liquids/water")
        (modifier "ROT0"))
      (position -76 464 -100)
      (repeat 6)
      (speed 50))
    (liquid 
      (surface 
        (image "liquids/water")
        (modifier "ROT0"))
      (position 808 278 -100)
      (repeat 8)
      (speed 50))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/twist")
        (modifier "ROT0"))
      (position 352 275 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch1")
        (modifier "ROT0"))
      (position -14 483 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch6")
        (modifier "ROT0"))
      (position 689 129 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch5")
        (modifier "ROT0"))
      (position 508 440 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch3")
        (modifier "ROT0"))
      (position -86 465 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch3")
        (modifier "ROT0"))
      (position 504 437 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch2")
        (modifier "ROT0"))
      (position 728 189 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch2")
        (modifier "ROT0"))
      (position -47 322 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch2")
        (modifier "ROT0"))
      (position 635 509 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch5")
        (modifier "ROT0"))
      (position 421 479 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch5")
        (modifier "ROT0"))
      (position 143 480 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/stick3")
        (modifier "ROT0"))
      (position 49 208 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/carabatree")
        (modifier "ROT0"))
      (position 590 176 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/carabatreemedium")
        (modifier "ROT0"))
      (position -3 187 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/green1")
        (modifier "ROT0"))
      (position 100 467 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/green2")
        (modifier "ROT0"))
      (position 621 56 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/green2")
        (modifier "ROT0"))
      (position 527 -6 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/green2")
        (modifier "ROT0"))
      (position 292 484 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/green2")
        (modifier "ROT0"))
      (position 99 364 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/green2")
        (modifier "ROT0"))
      (position 195 422 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/green2")
        (modifier "ROT0"))
      (position 383 539 0))
    (groundpiece 
      (type "solid")
      (surface 
        (image "groundpieces/solid/misc/metalplate_small")
        (modifier "ROT0"))
      (position 780 278 0))
    (groundpiece 
      (type "solid")
      (surface 
        (image "groundpieces/solid/misc/metalplate_small")
        (modifier "ROT0"))
      (position 780 310 0))
    (groundpiece 
      (type "solid")
      (surface 
        (image "groundpieces/solid/misc/metalplate_horiz")
        (modifier "ROT0"))
      (position 1004 310 0))
    (groundpiece 
      (type "solid")
      (surface 
        (image "groundpieces/solid/misc/metalplate_horiz")
        (modifier "ROT0"))
      (position 940 310 0))
    (groundpiece 
      (type "solid")
      (surface 
        (image "groundpieces/solid/misc/metalplate_horiz")
        (modifier "ROT0"))
      (position 876 310 0))
    (groundpiece 
      (type "solid")
      (surface 
        (image "groundpieces/solid/misc/metalplate_horiz")
        (modifier "ROT0"))
      (position 812 310 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch4")
        (modifier "ROT0"))
      (position 737 314 0))
    (groundpiece 
      (type "solid")
      (surface 
        (image "groundpieces/solid/misc/metalplate_large")
        (modifier "ROT0"))
      (position 350 279 0))
    (entrance 
      (position 178 72 0)
      (type "woodthing")
      (direction "left")
      (release-rate 25))
    (exit 
      (owner-id 0)
      (position 882 531 0)
      (surface 
        (image "exits/stone")
        (modifier "ROT0")))
    (hotspot 
      (surface 
        (image "entrances/woodthing_nmov")
        (modifier "ROT0"))
      (position 102 -62 100)
      (speed -1)
      (parallax 1))
    (liquid 
      (surface 
        (image "liquids/water")
        (modifier "ROT0"))
      (position 1 578 100)
      (repeat 32)
      (speed 30))))
;; EOF ;;
