;; generated by xml2sexpr.rb convert script from '../../../pingus_sdl/data/levels/./playable/jungle-airline-timpany.pingus'
(pingus-level 
  (version 2)
  (head 
    (levelname "Penguin Airlines")
    (description "
")
    (author "Craig Timpany, timpany@pingus.cx")
    (number-of-pingus 100)
    (number-to-save 85)
    (time 9000)
    (difficulty 5)
    (playable 1)
    (comment )
    (actions 
      (basher 5)
      (bridger 5)
      (climber 5)
      (digger 1)
      (floater 30)
      (jumper 30))
    (music "none")
    (levelsize 864 1216))
  (objects 
    (surface-background 
      (surface 
        (image "textures/enchantedjungle")
        (modifier "ROT0"))
      (color 0.8 1.2 1.2 0.2)
      (scroll-x 0.0)
      (scroll-y 0.0)
      (para-x 0.5)
      (para-y 0.5)
      (stretch-x 0)
      (stretch-y 0)
      (position 0 0 -150))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch6")
        (modifier "ROT0"))
      (position 601 717 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/pfosten")
        (modifier "ROT0"))
      (position 599 1049 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/stoneblock3")
        (modifier "ROT0"))
      (position 459 1070 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/industrial/pipe2")
        (modifier "ROT0"))
      (position 278 744 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/industrial/pipe2")
        (modifier "ROT0"))
      (position 278 681 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/industrial/pipe2")
        (modifier "ROT0"))
      (position 278 713 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/real/real7")
        (modifier "ROT0"))
      (position 511 950 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch5")
        (modifier "ROT0"))
      (position -21 456 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch3")
        (modifier "ROT0"))
      (position 404 567 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch3")
        (modifier "ROT0"))
      (position 599 198 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch6")
        (modifier "ROT0"))
      (position 259 595 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch6")
        (modifier "ROT0"))
      (position 398 597 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch6")
        (modifier "ROT0"))
      (position 958 577 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch6")
        (modifier "ROT0"))
      (position 597 429 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/real/real23")
        (modifier "ROT0"))
      (position 754 968 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/real/real19")
        (modifier "ROT0"))
      (position 642 950 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/desert/platform")
        (modifier "ROT0"))
      (position 778 947 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/desert/platform")
        (modifier "ROT0"))
      (position 545 946 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/desert/platform")
        (modifier "ROT0"))
      (position 662 946 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/stoneblock3")
        (modifier "ROT0"))
      (position 583 911 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/stoneblock3")
        (modifier "ROT0"))
      (position 519 942 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/stoneblock3")
        (modifier "ROT0"))
      (position 489 1006 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/misc/stoneblock3")
        (modifier "ROT0"))
      (position 331 1043 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/jungle/branch4")
        (modifier "ROT0"))
      (position -159 480 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/desert/pillar2")
        (modifier "ROT0"))
      (position 358 1106 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/desert/pillar2")
        (modifier "ROT0"))
      (position 332 1106 0))
    (groundpiece 
      (type "transparent")
      (surface 
        (image "entrances/entrance")
        (modifier "ROT0"))
      (position 659 203 0))
    (groundpiece 
      (type "ground")
      (surface 
        (image "groundpieces/ground/real/real7")
        (modifier "ROT0"))
      (position 686 1022 0))
    (exit 
      (owner-id 0)
      (position 753 952 0)
      (surface 
        (image "exits/stone")
        (modifier "ROT0"))
      (owner-id 0))
    (entrance 
      (position 692 225 0)
      (type "generic")
      (direction "left")
      (release-rate 25)
      (owner-id 0))))
;; EOF ;;
