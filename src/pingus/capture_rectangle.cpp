...
CaptureRectangle::draw(SceneContext& sc)
{
  if (pingu && pingu->catchable())
  {
    ...
    {
      sc.color().draw(good, pingu->get_center_pos(), 1000);
    }
    else
    {
      sc.color().draw(bad, pingu->get_center_pos(), 1000);
    }
    ...
    {
      sc.color().draw(arrow_right, pingu->get_center_pos() + geom::foffset(0, 2), 1000);
    }
