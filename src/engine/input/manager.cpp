...
Manager::create_controller(std::filesystem::path const& filename)
{
  ...
  {
    log_warn("{}: 'controls' section missing", filename.string());
  }
  else
  {
    for (auto const& key : controls_mapping.get_keys())
    {
      ...
      {
        log_error("{}: mapping must contain object at {}", filename.string(), key);
      }
      else
      {
        ...
        {
          throw std::runtime_error(std::format("Manager: Unkown Element in Controller Config: {}", key));
        }
      }
    }
  }
